...
export function EcuadorMap({
    ...
    layer.on({
      click: () => {
        if (redistributionMode) {
          ...
          } else if (sourceProvince !== provinceId) {
  ...
  const provinceStyle = (feature: any) => {
    ...
    const isSource = sourceProvince === provinceId
    ...
    return {
      fillColor: isSource ? '#8b5cf6' : isSelected ? '#f59e0b' : `rgb(${30 + intensity * 180}, ${41 + intensity * 80}, 59)`,
      weight: isSelected || isSource ? 3 : isHovered ? 2 : 1,
      opacity: 1,
      color: isSource ? '#8b5cf6' : isSelected ? '#f59e0b' : isHovered ? '#64748b' : '#334155',
      fillOpacity: isSelected || isSource ? 0.6 : 0.3 + intensity * 0.2
    }
  }
    ...
    <div className="relative w-full h-full bg-card rounded-lg overflow-hidden border border-border">
      ...
      >
        ...
        <GeoJSONDynamic
          ...
          key={`geojson-${selectedProvince}-${sourceProvince}`}
        />
      ...
      {/* Controls */}
      <Card className="absolute top-4 left-4 p-3 z-[1000]">
        <div className="flex flex-col gap-2">
          ...
          >
            ...
            {redistributionMode ? 'Cancelar Redistribución' : 'Redistribuir Personal'}
          </Button>
          
          {redistributionMode && (
            <p className="text-[10px] text-muted-foreground">
              {!sourceProvince 
                ? '1. Selecciona provincia origen' 
                : '2. Selecciona provincia destino'}
            </p>
          )}
        </div>
      </Card>
